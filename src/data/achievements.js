...
function weaponCount(m) {
  let n = 1; // 標準彈預設擁有
  for (const w of ["homing", "laser", "chain", "flame"]) if (m.weaponsOwned && m.weaponsOwned[w]) n++;
  return n;
}
...
export const ACHIEVEMENTS = [
  ...
  { id: "arms", name: "軍火庫",   icon: "cannon",reward: 120, desc: "解鎖全部 5 種武器", check: (m) => weaponCount(m) >= 5, prog: (m) => [weaponCount(m), 5] },
